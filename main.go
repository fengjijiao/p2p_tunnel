...
func main() {
        ...
        flag.StringVar(&conf.ServerListenPort,"port",":9000","Server running port")
        flag.StringVar(&conf.ClientConnectAddress,"connect","127.0.0.1:9000","Server address")
        ...
        if conf.isServer && conf.isClient {
                ...
                runAsServer(conf.ServerListenPort)
        }else if !conf.isServer && conf.isClient {
                runAsClient(conf.ClientConnectAddress)
        }else{
...
func runAsClient(CONNECT string) {
        srcAddr := &net.UDPAddr{IP: net.IPv4zero, Port: 19982} // 注意端口必须固定
